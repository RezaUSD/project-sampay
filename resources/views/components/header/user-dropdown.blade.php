<div class="relative" x-data="{
    ...
    <button
        ...
        type="button"
    >
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11 bg-brand-100 dark:bg-brand-900/30 flex items-center justify-center">
            <span class="text-brand-600 dark:text-brand-400 font-bold text-sm">{{ substr(auth()->user()?->nama_lengkap ?? 'A', 0, 1) }}</span>
        </span>

       <span class="block mr-1 font-medium text-theme-sm">{{ auth()->user()?->nama_lengkap ?? 'Admin' }}</span>
    ...
    >
        <!-- User Info -->
        <div>
            <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ auth()->user()?->nama_lengkap ?? 'Admin' }}</span>
